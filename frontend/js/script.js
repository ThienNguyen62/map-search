...
window.addEventListener("error", function (ev) {
  ...
  try {
    ...
    if (res)
      res.innerHTML = `<p style="color:#f88"><strong>Lỗi JavaScript:</strong> ${String(ev.error || ev.message || ev).replace(/</g, "&lt;")}</p>`;
    const mapEl = document.getElementById("map");
    if (mapEl)
      mapEl.innerHTML = `<div style="padding:20px;color:#333;background:#fff;border-radius:8px;margin:10px;">Lỗi JavaScript ngăn bản đồ hiển thị. Mở Console (F12) để xem chi tiết.</div>`;
  } catch (e) {}
});
window.addEventListener("unhandledrejection", function (ev) {
  ...
  try {
    const res = document.getElementById("result");
    if (res)
      res.innerHTML = `<p style="color:#f88"><strong>Unhandled promise rejection:</strong> ${String(ev.reason).replace(/</g, "&lt;")}</p>`;
  } catch (e) {}
});
...
function loadGraph() {
  ...
  const tryLoadStations = (index) => {
    if (index >= stationsSources.length) {
      document.getElementById("result").innerHTML =
        "<p>Không thể tải dữ liệu stations.json. Hãy chạy backend hoặc đặt file stations.json trong thư mục frontend.</p>";
      return;
    }
  ...
  const tryLoadEdges = (index) => {
    if (index >= edgesSources.length) {
      document.getElementById("result").innerHTML =
        "<p>Không thể tải dữ liệu edges.json. Hãy chạy backend hoặc đặt file edges.json trong thư mục frontend.</p>";
      return;
    }
...
async function computeAndShowFullRoute() {
  ...
  if (selectedStationIds.from) {
    ...
    fromLabel = graphById[fromStationId]
      ? graphById[fromStationId].name
      : fromLabel;
  } else if (customPoints.from) {
    const best = findNearestStation(
      customPoints.from.lat,
      customPoints.from.lon,
    );
    if (!best) {
      document.getElementById("result").innerHTML =
        "<p>Không có dữ liệu ga để tìm ga gần nhất.</p>";
      return;
    }
    fromStationId = best.id;
    walkingFrom = {
      distanceKm: best.distanceKm,
      time: best.walkMinutes,
      station: best.station,
      point: customPoints.from,
    };
  ...
  if (!fromStationId || !toStationId) {
    document.getElementById("result").innerHTML =
      "<p>Vui lòng chọn điểm đi và điểm đến (ga hoặc vị trí trên bản đồ).</p>";
    return;
  }
  ...
  if (isStationClosed(fromStationId) || isStationClosed(toStationId)) {
    ...
    if (isStationClosed(fromStationId)) {
      messages.push(
        `Ga đi ${graphById[fromStationId]?.name || fromStationId} đang đóng.`,
      );
    ...
    if (isStationClosed(toStationId)) {
      messages.push(
        `Ga đến ${graphById[toStationId]?.name || toStationId} đang đóng.`,
      );
    ...
    document.getElementById("result").innerHTML =
      `<p style="color:#b42318"><strong>${messages.join(" ")}</strong></p>`;
    return;
  }
  ...
  if (metroRoute) {
    if (Array.isArray(metroRoute.routes)) {
      ...
      routesList = [
        {
          path: metroRoute.path,
          metro_time: metroRoute.cost || 0,
          transfers: 0,
          stops: (metroRoute.path || []).length,
          lines: [],
        },
      ];
    }
  ...
  if (!routesList.length) {
    document.getElementById("result").innerHTML =
      "<p>Không tìm thấy lộ trình metro.</p>";
    return;
  }
    ...
    walkingLegPromises.push(
      (async () => {
        ...
        const m = L.circleMarker([p.lat, p.lon], {
          ...
          `<strong>Điểm đi</strong><br/>Đi bộ đến: ${s.name}<br/>${formatDistanceKm(res.distance || walkingFrom.distanceKm)} - ${formatMinutes(res.timeMin || walkingFrom.time)}`,
        );
    ...
    walkingLegPromises.push(
      (async () => {
        ...
        const m = L.circleMarker([p.lat, p.lon], {
          ...
          `<strong>Điểm đến</strong><br/>Từ: ${s.name}<br/>${formatDistanceKm(res.distance || walkingTo.distanceKm)} - ${formatMinutes(res.timeMin || walkingTo.time)}`,
        );
        m.addTo(walkingLayer);
        if (res && res.bounds) allBounds.push(res.bounds);
      })(),
...
function findStationIdByName(name) {
  ...
  const partial = graph.stations.find((st) =>
...
function dijkstra(startId, endId) {
  ...
  while (visited.size < nodes.length) {
    ...
    nodes.forEach((node) => {
      if (
        !visited.has(node) &&
        (current === null || costs[node] < costs[current])
      ) {
        current = node;
      }
    });
...
function findNearestStation(lat, lon) {
  if (!graph || !Array.isArray(graph.stations) || graph.stations.length === 0)
    return null;
  let best = null;
...
function clearCustomPoint(markerType) {
  if (customPointMarkers[markerType]) {
    try {
      customPointMarkers[markerType].remove();
    } catch (e) {}
...
function renderRoute(edges) {
  ...
  edges.forEach((edge) => {
    ...
    const to = graphById[edge.to];
    if (!from || !to) return;
    const edgeColor = blockedLines.has(edge.line)
      ? "#d00000"
      : lineColors[edge.line] || "#d90429";
    const polyline = L.polyline(
      [
        [from.lat, from.lon],
        [to.lat, to.lon],
      ],
      { color: edgeColor, weight: 6, opacity: 0.85 },
    );
...
function removeRoute(index) {
  ...
  try {
    map.removeLayer(routeLayers[index].layerGroup);
  } catch (e) {}
  // remove from arrays
  ...
  if (lastRoutesList.length === 0) {
    ...
    walkingLayer.clearLayers();
    document.getElementById("result").innerHTML =
      `<p>Không có tuyến nào để hiển thị.</p>`;
    return;
  }
  ...
  const elem = document.querySelector(
    `.route-item[data-idx="${selectedRouteIndex}"]`,
  );
  ...
  if (sel) {
    ...
    showResult(
      { path: sel.meta.path, cost: sel.meta.metro_time, edges },
      lastFromDisplay,
      lastToDisplay,
      lastWalkingFrom,
      lastWalkingTo,
...
function updateRoutesUI() {
  ...
  const listHtml = routesList
    .map((r, i) => {
      ...
      const note = blocked
        ? ' <span style="color:#d00000;font-weight:700;">[Tuyến bị cấm]</span>'
        : "";
      const style = blocked
        ? "cursor:not-allowed;opacity:0.6;background:#fff0f0;padding:6px;border-bottom:1px solid #eee;display:flex;justify-content:space-between;align-items:center;"
        : "cursor:pointer;padding:6px;border-bottom:1px solid #eee;display:flex;justify-content:space-between;align-items:center;";
      return (
        `<li data-idx="${i}" class="route-item" style="${style}">` +
...
async function fetchAndDrawWalkingLeg(point, station, opts = {}) {
  ...
  if (routeGeo && routeGeo.coordinates && routeGeo.coordinates.length) {
    ...
    return {
      bounds: pl.getBounds(),
      distance: routeInfo ? routeInfo.distance / 1000 : null,
      timeMin: routeInfo ? Math.round(routeInfo.duration / 60) : null,
    };
  }
  ...
  const straight = L.polyline(
    ...
    { color, weight: 4, dashArray: "6,8", opacity: 0.9 },
  );
...
function renderAllRoutes(routes, walkingFrom, walkingTo) {
  ...
  routes.forEach((r, idx) => {
    ...
    const defaultColor = routeBlocked
      ? "#d00000"
  ...
  if (walkingFrom) {
    ...
    L.polyline(
      ...
      { color: "#2f4f4f", weight: 4, dashArray: "6,8", opacity: 0.9 },
    ).addTo(walkingLayer);
  ...
  routeLayers.forEach((r) => {
    try {
      boundsParts.push(r.layerGroup.getBounds());
    } catch (e) {}
  });
  try {
    if (walkingLayer.getLayers().length)
      boundsParts.push(walkingLayer.getBounds());
  } catch (e) {}
...
function getRouteOperationStatus(route) {
  ...
  const blockedRouteLines = Array.from(
    new Set(
      route.edges
        .map((edge) => edge.line)
        .filter((line) => blockedLines.has(line)),
    ),
  );
...
function getClosedStationIdsFromAdminState() {
  try {
    ...
    return new Set(
      parsed.stations.filter((st) => st && st.closed).map((st) => st.id),
    );
  } catch (error) {
    console.warn("Cannot parse station admin state from localStorage:", error);
    return new Set();
...
function showResult(
  ...
  const blockedLinesText =
    operationStatus.blockedRouteLines.length > 0
  ...
  if (walkingFrom) {
    walkHtml.push(
      `<li><strong>Điểm đi → Đi bộ → Ga gần nhất:</strong> ${formatDistanceKm(walkingFrom.distanceKm)} , ${formatMinutes(walkingFrom.time)} → <strong>${walkingFrom.station.name}</strong></li>`,
    );
  }
  if (walkingTo) {
    walkHtml.push(
      `<li><strong>Ga gần nhất → Đi bộ → Điểm đến:</strong> ${formatDistanceKm(walkingTo.distanceKm)} , ${formatMinutes(walkingTo.time)} → <strong>${walkingTo.station.name}</strong></li>`,
    );
  }
  ...
  const stationLinks = container.querySelectorAll
    ? container.querySelectorAll(".station-link")
...
function setMapMode(mode) {
  ...
  document.querySelectorAll(".map-switch button").forEach((btn) => {
    btn.classList.toggle(
      "active",
      btn.textContent.trim() === (mode === "satellite" ? "Vệ tinh" : "Bản đồ"),
    );
  });
}
...
map.on("click", (e) => {
  ...
  if (markerType === "from")
    document.getElementById("fromStation").value = label;
