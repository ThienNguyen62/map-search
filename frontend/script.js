  ...
  const tryLoad = (index) => {
    if (index >= sources.length) {
      document.getElementById("result").innerHTML =
        "<p>Không thể tải dữ liệu điểm ga. Hãy chạy backend hoặc đặt file metro_graph.json trong thư mục frontend.</p>";
      return;
    }
...
async function findRoute() {
  ...
  if (!fromInput || !toInput) {
    document.getElementById("result").innerHTML =
      "<p>Vui lòng chọn ga hợp lệ từ danh sách gợi ý.</p>";
    return;
  }
  if (fromInput === toInput) {
    document.getElementById("result").innerHTML =
      "<p>Ga đi và ga đến phải khác nhau.</p>";
    return;
  }
  ...
  try {
    ...
    if (!fromId || !toId) {
      document.getElementById("result").innerHTML =
        "<p>Không tìm thấy ga trong dữ liệu hiện có.</p>";
      return;
    }
    const route = dijkstra(fromId, toId);
    showResult(route, fromInput, toInput);
